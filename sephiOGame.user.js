...
//try {
    ...
    LANG_nouveaute_update = isFR ? ' - Configuration du nombre de slot de libre <br> - Correction d\'anomalie lorsque lune vide'
    : ' - Set the free slot <br> - Bugs correction on moon empty';
    ...
    if (data !== null && data.split(":").length > 2) {
        ...
        eject_url+= (eject_onLune)?'&type=3&mission=3&eject=yes':'&type=1&mission=3&eject=yes';
        ...
        if (rand(1,20) == 1 && !gup('page').match('fleet')) {
            ...
            eject_url = 'http://'+univers+'/game/index.php?page=fleet1&galaxy='+eject_gal+'&system='+eject_sys+'&position='+eject_pla
            eject_url+= (eject_onLune)?'&type=3&mission=3':'&type=1&mission=3';
            $.ajax(eject_url, {
                dataType: "text",
                type: "POST",
    ...
    function getServerLastVer() {
        ...
        waitServAnswer = setInterval(function() {
            if (readCookie('infoServ', 'all') !== ''){
                ...
                //document.geElementById('servQuestion').src = "";
            ...
            success: function(dateESP) {
                ...
                //} else if (!dateESP.response.message.match('Erreur') && !dateESP.response.message.match('erreur')) {
                    ...
                    if (want_a_AA && nb_tries>5 && next_id==0) {
                        ...
                        if (gup('startAA') == '1') {
                            createCookie('progTime', time() + 60*60*1000, 1,'AA' ); // re-essaye dans 1heure
                            createCookie('isProg', 'oui', 1,'AA' );
    ...
    if (gup('page') !== 'traderOverview' && gup('page') !== 'premium' && gup('page') !== 'galaxy' && gup('page') !== 'highscore' && gup('page') !== 'fleet1' && gup('page') !== 'fleet2' && gup('page') !== 'fleet3' && readCookie('isProg','AA') == 'oui' && readCookie('progTime','AA') !== null) {
        ...
        if (time_restant >= 0) {
            ...
            //Imp2Toulouse: MalWritten correction
            //data += '<p style="width:600px;height:20px;white-space: nowrap">Auto-Attaque <b>prévue dans <span id="countdonwAA">'+get_cool_time(time_restant/1000)+'</span></b>'+repeat_text;
    ...
    function countdownRetour() {
        ...
        //if (t>0) {document.getElementById('countdonwRetour').innerHTML = get_cool_time(t/1000);setTimeout(countdownRetour,1000);}
    ...
    if (readCookie('retour_auto', 'eject') == 'oui') {
        ...
        if (time_restant > 0) {
            ...
            //Imp2Toulouse malwritten correction countdonwRetour by countdownRetour
            //data += '<p style="width:600px;height:20px;white-space: nowrap"><b>Demande du retour de la flotte ejectée dans <span id="countdonwRetour">'+get_cool_time((retour_time - time() + parseInt(readCookie('ejection_time', 'eject')))/1000)+'</span></b></p>';
    ...
    if (gup('page') == "fleet1" && gup('eject') == 'yes') {
        ...
        //base of url
        form.action +="&union=0&eject=yes";
        //add type and mission
        form.action +="&type="+((eject_onLune)?3:1)+"&mission=3";
        //add Fleets
        if (eject_all) 
            setTimeout(function(){setMaxIntInput("form[name=shipsChosen]", {"202":99999,"203":99999,"204":99999,"205":99999,"206":99999,"207":99999,"208":99999,"209":99999,"210":99999,"211":99999,"213":99999,"214":99999,"215":99999});}, 400);
        else 
            setTimeout(function(){setMaxIntInput("form[name=shipsChosen]", {"202":99999,"203":99999,"208":99999,"209":99999,"210":99999});}, 400);
        
        //add target coord
        //form.action +="&galaxy="+form.galaxy.value+"&system="+form.system.value+"&position="+form.position.value;
        form.action +="&speed=1";
    ...
    if (gup('page') == "fleet1" && gup('auto') == 'yes') {
        ...
        if (has_flotte && readCookie('AA_leave_slot','AA') == 'oui') {
            ...
            // 1 specify a one only slot reserved
            //if (max_flotte - nb_flotte <= 1) has_flotte = false;
    ...
    if (gup('page') == 'messages') {
        ...
        //data +document.getElementById('rap_general_coord_'+GLOB_rgID).innerHTML.replace('[','').replace(']','').split(':')
        ...
        data += '</div>';
        data += '</div>';
        // Imp2Toulouse- Detection issue message and garbage when tabs block is modified.... To investigate, maybe redefine onclick for this 2 tabs
    ...
    */
        ...
        document.getElementById('eject_save_button').onclick = function () {
            ...
            if (document.getElementById('auto_eject').value == '1mins') eject_data += '1mins:';
