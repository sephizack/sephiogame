...
function save_list_in_cookies() {
    ...
    if ((gup('page') == "fleet2" || gup('page') == 'messages' || gup('page') == 'galaxy') 
        ...
        if (readCookie('add_racc','all') != null && parseInt(readCookie('add_racc','all')) > 0) {
            ...
            save_important_vars();
            blit_message(persistedData["frigos"][cur_nb][0]+' a été <span style="float: none;margin: 0;color:#109E18">ajouté à vos frigos</span> !');
...
if (gup('sephiScript') == '1') {
    ...
    for (i=0;i<persistedData["frigos"].length;i++) {
        ...
        sephi_frigos_data+='<th style="text-align:left;width:55px;"><input type="text" style="width:55px;font-size:x-small;position:relative;margin-left:5px;left:0px;text-align:center;" id="frig_name_'+i+'" value="'+persistedData["frigos"][i][0]+'" /></th>';
