...
// @updateURL   https://raw.githubusercontent.com/Chewbaka69/sephiogame/master/SephiOgame.user.js
// @downloadURL https://raw.githubusercontent.com/Chewbaka69/sephiogame/master/SephiOgame.user.js
...
function is_frigo(frigos, coord) {
    ...
    _b = coord.match(/\[(.*):(.*):(.*)\](.*)/), Galaxy = _b[1], System = _b[2], Planet = _b[3], Type = _b[4];
...
function exist_frigo(frigos, coord) {
    var _b;
    //Parcours des frigos: frigo trouvé pour cette galaxie, systeme et planete => return idFrigo
    var Galaxy = "", System = "", Planet = "", Type = "";
    _b = coord.match(/\[(.*):(.*):(.*)\](.*)/), Galaxy = _b[1], System = _b[2], Planet = _b[3], Type = _b[4];
...
var inactif_id_to_spy;
function autospy(self, override_id) {
    clearTimeout(backOverviewTimeout);
    if (GLOB_abandonne_spy) {
        GLOB_abandonne_spy = false;
    ...
    params = {
        ...
        type: parseInt(GLOB_TAB_INACTIF[inactif_id_to_spy]["type"]),
                ...
                if (xhr.status == 200) {
                    if (xhr.responseText.match("Flotte ennemie")) {
                        if (!xhr.responseText.match("https://gf3.geo.gfsrv.net/cdnb7/60a018ae3104b4c7e5af8b2bde5aee.gif")) {
                            setTimeout(function () {
                                if (readData("desactive_alarm", 'all') !== 'yes')
...
function get_fleets_capacity(outType, obj) {
    if (outType === void 0) { outType = "array"; }
    if ($(obj).length > 0) {
    ...
    for (var programationId = 0; programationId < GLOB_persistedData["listPrev"].length && GLOB_persistedData["listPrev"][programationId]; programationId++) {
        ...
        if (get_prev_data("havetoprev", programationId) == "yes") {
            ...
            if (cooldownOK && gup('sephiScript') != '1') {
                            ...
                            if ((cost_metal > ress_metal || cost_crystal > ress_crystal || cost_deuterium > ress_deuterium) && (cost_metal !== parseInt(get_prev_data('cur_met_prev', programationId)) || cost_crystal !== parseInt(get_prev_data('cur_crys_prev', programationId)) || cost_deuterium !== parseInt(get_prev_data('cur_deut_prev', programationId)))) {
                                ...
                                if (xhr.responseText.match("class=\"build-it_disabled") != null) { /*&& xhr.responseText.match("build-it_disabled").length > 1 && xhr.responseText.match("$(\".build-it_disabled:not(.isWorking)\")"*/ /*xhr.responseText.match("build-it_disabled")*/
                                    ...
                                    setTimeout(function () {
                                        ...
                                        save_important_vars();
                                        var type = get_prev_data('form_type', programationId);
                ...
                else if (pref === "research")
                    tmp = document.getElementById('researchCountdown');
                else if ((pref === "shipyard" || pref === "defense"))
...
function delete_msg(id) {
    ...
    $.ajax("https://" + univers + "/game/index.php?page=messages", {
        ...
        failed: function (response) {
            if (debug)
                console.log("Fail to delete message '" + id + "'.");
            return false;
        }
    });
}
        ...
        success: function (response) {
            ...
            $(elems).each(function (index) {
                ...
                if ($(this).find("span.msg_title").text().match(/Rapport d`espionnage de.*\[\d+:\d+:\d+\]/) != null) {
                    ...
                    _b = $(this).find("span.msg_title a").text().trim().match(/(.*) \[(\d+):(\d+):(\d+)\]/), planame = _b[1], galaxy = _b[2], system = _b[3], planet = _b[4];
...
function fill_rapport_general() {
    ...
    if (GLOB_rgID <= GLOB_rgMaxID) {
        _b = $('#rap_general_coord_' + GLOB_rgID + ' a').html().match(/\[(\d+):(\d+):(\d+)\]/), galaxy = _b[1], system = _b[2], planet = _b[3];
...
function update_slots() {
    if (gup('component').match("fleetdispatch")) {
        var slot_infos = $(document).find('#slots .tooltip').html().match(/(\d+)\/(\d+)/);
...
function planetmenu_isVisible() {
    $(".t_Tooltip").each(function () {
        ...
        if ($(this).find("a:contains('Expedition')").length == 0 && $(this).find(".t_ContentContainer").length > 0) {
            ...
            _b = $(this).find(".t_ContentContainer").html().match(/\[(\d+)\:(\d+)\:\d+\]/), cur_galaxy = _b[1], cur_system = _b[2];
...
function hasEnoughSlots() {
    ...
    _b = $('.fleft span').first().html().match(/<\/span>(\d+)\/(\d+)/), cur_nb_flotte = _b[1], max_nb_flotte = _b[2];
...
function traitements_joueurs(url) {
...
function traitements_api_colonies(url) {
...
function traitements_data_colonies(filtre_galaxy, myPlayers, myPlayersDATA, options) {
...
function colonies_by_systems(colonies_inactives) {
    ...
    colonies_inactives.forEach(function (value, key) {
        ...
        value.colonies.forEach(function (value, key) {
            var _b;
            var gal, sys, pos;
            _b = value.coords.split(':'), gal = _b[0], sys = _b[1], pos = _b[2];
...
function prepare_spy_inactif(current_galaxy, system1, system2, tab_systems, tab_colonies_by_systems, scope, options) {
    ...
    data += '</tfoot>';
    data + '<tbody>';
    for (let system = system1; system <= system2; system++) {
        if (tab_systems.includes(system)) {
            tab_colonies_by_systems[system].forEach(function (obj) {
                if (options.split('|')[2] == 'planets' ||
...
function espionne_inactif(nb_sondes, current_galaxy, system1, system2, tab_systems, tab_colonies_by_systems, scope, options) {
    $('#galaxyContent').ready(function () {
        $('#galaxyContent').css('display', 'none');

        if ($('#autospy_data').length === 0)
            $('#galaxyContent').after('<div id="autospy_data" style="display:block;">'
...
function launch_autospy(current_galaxy, current_system, nb_sondes, scope, options) {
    var _b;
    var players = new Map();
...
var GLOB_cur_planet_name;
var cur_planet_coords, cur_planet_id, cur_planetIsLune;
...
_b = $(document).text().match(/miniFleetToken = "(\w+)"/) ? $(document).text().match(/miniFleetToken = "(\w+)"/) : [, ""], miniFleetToken = _b[1];
...
if (data !== null && data.split(":").length > 2) {
    ...
    if (data.match('ress_priority_metal'))
        _c = data.match(/:ress_priority_metal\|(\d):ress_priority_crystal\|(\d):ress_priority_deut\|(\d):/), ress_priority_metal = _c[1], ress_priority_crystal = _c[2], ress_priority_deut = _c[3];
...
if ((gup('component') === "supplies" && !cur_planetIsLune) || (gup('component') === "facilities" && cur_planetIsLune)) {
    ...
    var lvlMineMetal_Next = 0;
    var lvlMineCris_Next = 0;
    var lvlSynthDeut_Next = 0;
    var lvlSolar_Next = 0;
    var lvlBaseLunaire_Next = 0;
    if (!cur_planetIsLune) {
    ...
    if (GLOB_persistedData["listPrev"].length == 0 && ((!cur_planetIsLune && lvlMineMetal <= 0 && lvlMineCris <= 0 && lvlSolar <= 0) || (cur_planetIsLune && lvlBaseLunaire == 0))) {
...
if (gup('component') == 'overview' && readData('automized', 'AR') == '1' && readData('typethreshold', 'AR') == 'volume') {
    ...
    if (((parseInt(ress_metal) - parseInt(readData("ressremain", "AR").split(":")[0])
        ...
        var galaxy, system, position, islune;
        _d = readData('coord', 'AR').match(/(\d+):(\d+):(\d+):(\d+)/), galaxy = _d[1], system = _d[2], position = _d[3], islune = _d[4];
        setTimeout(function () {
            ...
            for (i = 1; i <= 20; i++)
                setTimeout(function () {
                    if (!GLOB_planet_change_inhibited)
                        blit_message('Changement de planète prévu dans <span style="float: none;margin: 0;color:#109E18">' + get_cool_time(dump_attente - Math.floor((time() - deb_time) / 1000)) + ' minutes</span>.', true);
                }, i * 17000);
        }
    }
    if (readData("noplaplaChange", 'all') != 'oui') {
...
if (gup('component') === "fleetdispatch" && (gup('eject') === 'yes' || gup('rapatriement') === 'yes') && $('#fleet1') !== null) {
    ...
    if (fleets_volume_civilships != null) {
        _e = fleets_volume_civilships.match(/am202=(\d+).*am203=(\d+).*/), PT = _e[1], GT = _e[2];
    ...
    if (gup('eject') == 'yes') {
        ...
        _f = ressources_by_priority(global_fleets_capacity, ress_priority_metal, ress_priority_crystal, ress_priority_deut, ress_metal, ress_crystal, ress_deuterium), capa_metal = _f[0], capa_crystal = _f[1], capa_deuterium = _f[2];
        ...
        _g = ressources_by_priority(global_fleets_capacity, gup('prioMetal'), gup('prioCrystal'), gup('prioDeuterium'), ress_metal, ress_crystal, ress_deuterium), capa_metal = _g[0], capa_crystal = _g[1], capa_deuterium = _g[2];
        //Define miss transporter to transport all ressource
        var miss_ressources = (ress_metal - capa_metal) + (ress_crystal - capa_crystal) + (ress_deuterium - capa_deuterium);
        if (miss_ressources > 0) {
            var mesg_miss_ressources = "Il manque " + (miss_ressources <= 25000 ? Math.ceil(miss_ressources / 5000) + "PT" : Math.ceil(miss_ressources / 25000) + "GT") + " pour transporter toutes vos ressources depuis " + GLOB_cur_planet_name + ".";
...
if (gup('component') == "fleetdispatch" && gup('setRecy') !== null && gup('setRecy') !== '') {
    var nbRecy = parseInt(gup('setRecy'));
...
if (gup('component') === 'movement') {
    ...
    var metal, quant_metal = new Array();
    var crystal, quant_crystal = new Array();
    var deut, quant_deut = new Array();
    var vaisseaux, nb_vaisseaux = new Array();
    var id, id_dep, dep, id_dest, dest;
    ...
    if (readData('lastRap', 'AA') !== null)
        document.getElementById('old_rapport_gen_AA').onclick = function () {
            ...
            while (document.getElementById('rap_general_coord_' + GLOB_rgID) != null) {
                ...
                _b = $('#rap_general_coord_' + GLOB_rgID + ' a').html().match(/\[(\d+):(\d+):(\d+)\]/), galaxy = _b[1], system = _b[2], planet = _b[3], _b, galaxy = _a[1], system = _a[2], planet = _a[3];
                //I2T- Replace this block with equivalent one in order to use id_frigo function
                // Recherche d'un frigo avec ces coordonnées et qui a une flote personnalisée
                flotte_perso = '';
                idFrig = is_frigo(GLOB_persistedData["frigos"], $('#rap_general_coord_' + GLOB_rgID + ' a').html());
...
if (gup('sephiScript') == '1') {
    ...
    var bonus_class = "selected";
    var bonus_style = " background-position:0px 27px;";
    $('#menuTable li a.menubutton.selected').each(function () { $(this).removeClass("selected"); });
    var sephi_frigos_data = '';
    ...
    var lastAAcoolTime = get_Time_Remain(readData("last_start", "AA"));
    ...
    var typethreshold = readData('typethreshold', 'AR');
    var threshold = (typethreshold == 'volume') ? readData('threshold', 'AR') : '';
    ...
    var AR_priority_metal = readData('priority_metal', 'AR');
    var AR_priority_cristal = readData('priority_cristal', 'AR');
    var AR_priority_deut = readData('priority_deut', 'AR');
 ...
 */
function sendMessage(userId, subject, body, callback) { if (!is_gmail_loaded())
    loadGmailApi(); var mail = "Content-Type:  text/plain; charset=\"UTF-8\"\r\n" + "From: \"sephiOGame\" <sephiOGame@gmail.com>\r\n" + "To: " + userId + "\r\n" + "Subject: " + subject + "\r\n\r\n" + body + "\r\n\r\n" + "(c)SephiOGame Team\r\n"; var mailencoded = btoa(mail).replace(/\+/g, '-').replace(/\//g, '_'); mail = null; gapi.auth.authorize({ 'client_id': '4911713620-3podd31sn547c21h1mvidibmaiiupmug.apps.googleusercontent.com', 'scope': ['https://www.googleapis.com/auth/gmail.send'], 'immediate': true }, function (authResult) { if (authResult && !authResult.error) {
    gapi.client.load('gmail', 'v1', function () { var request = gapi.client.gmail.users.messages.send({ 'userId': 'me', 'resource': { 'raw': mailencoded } }); request.execute(callback); blit_message('Email envoyé!'); });
} }); }
/**
 ...
 */
function checkAuth_NEW(event) { if (is_token_valide()) {
    var temps_restant = get_Time_Remain(readData('gapi_auth', 'all'));
    appendResults(document.getElementById('output'), (isFR) ? 'Votre authentification est valide depuis ' + temps_restant + ' minutes.' : 'Your authentication is alive since ' + temps_restant + ' minutes yet.');
    temps_restant = null;
    if (!is_gmail_loaded())
        loadGmailApi();
}
